...
static void aKeyboardCB(unsigned char key, int x, int y) { callbacks->KeyboardCB(key, x, y); }
static void InitCallbacks()
{
	glutDisplayFunc(aRenderSceneCB); //GLUT interacts with window system
	glutIdleFunc(aIdleCB);
	glutKeyboardFunc(aKeyboardCB);
}
...
public:
	Main()
	{
		...
		pTexture = nullptr;
		pEffect = nullptr;
		directionalLight.Color = glm::vec3(1.0f, 1.0f, 1.0f); // цвет света (белый)
		directionalLight.AmbientIntensity = 0.5f; // общая яркость, фоновое освещение
		directionalLight.DiffuseIntensity = 0.2f; // сила рассеянного света
		directionalLight.Direction = glm::vec3(10.0f, 1.0f, 0.0f); // вектор направления рассеянного света
	}
	...
	bool Init()
	{
		...
		pTexture = new Texture(GL_TEXTURE_2D, "test9.jpg");
	...
	virtual void RenderSceneCB() override //draw
	{
		...
		p.Scale(0.3,0.3,0.3); // размер
		p.WorldPos(0.0f, 0.0f, 0);
		p.Rotate(0, Scale, 0); // как вращается


		glm::vec3 CameraPos(0.0f, 0.0f, -3.0f); // где находится камера 
		glm::vec3 CameraTarget(0.0f, 0.0f, 2.0f); // куда смотрит камера
		...
		sl[0].Position = glm::vec3(0.0f, 0.0f, 0.0f);
		sl[0].Direction = glm::vec3(sinf(Scale1), 0.0f, cosf(Scale1));
		sl[0].Attenuation.Linear = 0.1f;
		sl[0].Cutoff = 100.0f; 
		// значение отсекания - максимальный угол между направлением света и вектором до пикселей, которые еще попадут под влияние света

		sl[1].DiffuseIntensity = 0.5f; //интенсивность рассеивания
		sl[1].Color = glm::vec3(1.0f, 0.0f, 1.0f);
		sl[1].Position = -CameraPos;
		sl[1].Direction = -CameraTarget;
		sl[1].Attenuation.Linear = 0.1f; // затухание
		sl[1].Cutoff = 100.0f; // косинус угла, чем больше - тем большую площадь охватывает прожектор

		pEffect->SetSpotLights(2, sl);

		PointLight pl[3]; // точененый источник светит во все стороны, поэтому можем не указывать его направление
		pl[0].DiffuseIntensity = 0.3; // интенсивность (яркость) света
		pl[0].Color = glm::vec3(1.0f, 0.0f, 0.0f); // red
		pl[0].Position = glm::vec3(sinf(Scale1) * 10, 1.0f, cosf(Scale1) * 10);
		pl[0].Attenuation.Linear = 0.1;

		pl[1].DiffuseIntensity = 0.3;
		pl[1].Color = glm::vec3(0.0f, 1.0f, 0.0f); // green
		pl[1].Position = glm::vec3(sinf(Scale1 + 2.1f) * 10, 1.0f, cosf(Scale1 + 2.1f) * 10);
		pl[1].Attenuation.Linear = 0.1;

		pl[2].DiffuseIntensity = 0.3;
		pl[2].Color = glm::vec3(0.0f, 0.0f, 1.0f); // blue
		pl[2].Position = glm::vec3(sinf(Scale1 + 4.2f) * 10, 1.0f, cosf(Scale1 + 4.2f) * 10);
		pl[2].Attenuation.Linear = 0.1;
		...
		pEffect->SetMatSpecularIntensity(0); // интенсивность отражения
		pEffect->SetMatSpecularPower(0); // коэффициент отражения материала
...
private:
	...
	void CreateBuffers()
	{
		Vertex Vertices[4] =
		{
			//			коорд пирамиды				коорд текстуры
			Vertex(glm::vec3(0.5f, 1.0f, 0.0f), glm::vec2(0.0f, 0.0f)), 
			Vertex(glm::vec3(0.0f, -1.0f, 1.0f), glm::vec2(0.5f, 0.0f)),
			Vertex(glm::vec3(1.25f, -0.25f, 1.25f), glm::vec2(1.0f, 0.0f)),
			Vertex(glm::vec3(0.5f, -1.0f, -1.0f), glm::vec2(0.5f, 1.0f)),
		};
	...
	virtual void KeyboardCB(unsigned char key, int x, int y)
	{
		...
		case 'd': // яркость рассеянного света
